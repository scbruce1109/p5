...
function setup() {
  ...
  for (let i = 0;i<100;i++){
   var p = placePoint3D(placePoint3D(createVector(0,0,0), random(360),random(360),400), random(360),random(360),400)
   console.log(p)
   ding = new Rect3D(p,random(100,200),random(100,200))
   ding.rotate(radians(Math.floor(random(720)/90)*90),'x')
   ding.rotate(radians(Math.floor(random(720)/90)*90),'z')
   shapes.push(ding)
 }

 fill(360,.1)
 // for (let i = 0;i<shapes.length;i++){
 //   // shapes[i].rotate(PI/180,'x')
 //   shapes[i].rotate(PI/180*20,'z')
 //   // ls[i].scale(0.999)
 //   shapes[i].display(ging)
 // }


 sorted = sortPoints(shapes,ging)
 console.log(sorted)
 var range = (sorted[2] - sorted[1])*2

 for (let i = 0;i<sorted[0].length;i++){

   var d = p5.Vector.dist(sorted[0][i].center,ging.location);
   var p = ging.project([sorted[0][i].center])[0]
   var c = lerpColor(color(360),g2.getValue(p.x,p.y).c,Math.abs(d/range))
   console.log(d/range)
   c.setAlpha(0.05)
   // fill(c)
   sorted[0][i].c = c
