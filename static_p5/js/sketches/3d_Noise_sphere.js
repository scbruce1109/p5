...
function setup() {
  ...
  sort = []
  for (let i = 0;i<5;i++){
    pal.push(colors[Math.floor(random(colors.length))])
  }
  console.log(pal)

  createCanvas(600, 600);
  background('#d5f5ff');
  var cp = spherePoints(createVector(0,0,0),150,1000);
  movers = []
  for (let i = 0;i<cp.length;i++){
    m = new Mover3D(cp[i],2, new Sphere3D(cp[i],2))
    m.color = color(pal[Math.floor(random(pal.length))])
    m.color.setAlpha(.20)
    m.life = random(10,200)
    movers.push(m)
    sort.push(m.mesh)
  }
...
for (let i = 0;i<ding.length;i++){
  m = ding[i]
  var d = p5.Vector.dist(m.mesh.center,ging.location);
  // var c = lerpColor(m.color,color(255),Math.abs(d/range))
  var c = lerpColor2(m.color,color('#d5f5ff'),map(m.life,200,0,0,1), "MIX", null, null, LINEAR_)
  // m.addForce(createVector(random(-1,1),random(-1,1),random(-1,1)))////.z += map(noise(t),0,1,-1,1);  noise3D1(m.location) createVector(random(-1,1),random(-1,1),random(-1,1))
  m.addForce(noise3D1(m.location)) //createVector(random(-1,1),random(-1,1),random(-1,1)))
  // var c =
  c.setAlpha(0.30)
  stroke(c)
  fill(c)
