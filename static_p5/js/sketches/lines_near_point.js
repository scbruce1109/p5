...
lines = [];
  for (let i = 0;i<20;i++){
      var p = placePoint3D(p,random(360),random(360),randomGaussian(100,200));
      var p2 = createVector(p.x,p.y,p.z+randomGaussian(100,200))
      var lin = new Line3D([p,p2])
      lin.rotate(radians(45),'x')
      lines.push(lin)
      lin.c = palette.getColor(random(1))
      lin.c.setAlpha(10)
      // console.log(lines)
...
for (let i = 0;i<lines.length;i++){
  // lines[i].display(cam)
  stroke(color(lines[i].c))
  for (let j = 0;j<100;j++){
    var lineAngle = lines[i].getAngle(random(1))
    console.log(lineAngle)
