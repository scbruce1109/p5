...
function setup(){
  ...
  cg1.fillColor([color("#515e67"),color("#f1e0ac"),color('#ff9c5e'),color('#cc1d1d'),color('#0c2947')],params.points,600,color('#ffffff'))
...
function draw(){
  ...
  for (let i = 0;i<movers.length;i++){
    ...
    if (c[0] > 90){
      ...
      // cc.setAlpha(0.1)
      movers[i].display(color(255,0.05));
...
function paintStroke(x,y,width_,height_,numPoints){
  var zoop = generatePoints(x, y,random(width_),random(height_),numPoints)//Math.floor(random(3,8)))
...
function paintStroke2(x,y,width_,height_,numPoints){
  var p = placePoint(createVector(width/2,height/2),500,false)
  var zoop = generatePoints(x, y,random(width_),random(height_),4,45)
  var ding = new myShape(zoop,true)
  // ding.offsetPoints(random(10))
  ding.subdivide(2)
  console.log(ding.points)
