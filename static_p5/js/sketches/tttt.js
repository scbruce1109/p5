var e,r,e2,r2,g1,shapes,movers;
...
function setup() {
  ...
  r = new Sphere3D(createVector(0,0,0.01),75)
  // r = new Rect3D(createVector(0,0,0),20,20)
  ...
  e.smoothChaikin(4)
  e.translate(createVector(-200,0,0))

  e2 = e.copy()
  e2.translate(createVector(800,0,0))
  e3 = e.copy()
  e3.translate(createVector(800,0,0))



  shapes = arrayOnLine(r,e,50)
  var shapes2 = arrayOnLine(r,e2,50)
  var shapes3 = arrayOnLine(r,e3,50)

  shapes = shapes.concat(shapes2)

  g2.fillGradient(cp1)
  // g2.fillColor([color("#fefefe"),color("#feaca4"),color("#d4fdff"),color("#fec375")],pPoints,300,g1)

  // shapes = arrayOnLine(r,e,150)
  movers = []
  for (let i = 0;i<shapes.length;i++){
    var m = new Mover3D(shapes[i].center,5,shapes[i])
    movers.push(m)
  }
  console.log(shapes)

  g1.fillColor([color("#ff7a40"),color("#ffe040"),color("#169dff")],pPoints,200,color('#ffffff'))
  g1.display();
...
function draw() {
  ...
  stroke(0,100);
    ...
    var foopforce = createVector(1,0,0)
    var dingforce = createVector(0,0,0)
    // var ff =
    ...
    for (let i = 0;i<movers.length;i++){
      // movers[i].addForce(noise3D1(movers[i].mesh.center).setMag(0.2))
      // movers[i].addForce(createVector(0,0,1))
      // var ff = noise3D1(movers[i].center)
