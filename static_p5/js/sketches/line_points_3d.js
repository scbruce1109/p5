...
var p = createVector(0,0,0);
  ...
  l = new Line3D([createVector(0,0,0), pointOnGround(ging,30,10000)[1]])
  ...
  console.log(fovWidth(ging,10000))
  l2 = new Line3D([createVector(0,0,2000), pointOnGround(ging,0,10000)[1]])
  l2.subdivide(5)
  // l2.subdivide(1)
  t = 1000
  for (let i = 0;i<l2.faces[0].length;i++){
    l2.faces[0][i].z += l2.faces[0][i].z+noise(t)*500
    l2.faces[0][i].x += l2.faces[0][i].z+noise(t)*random(-2000,2000)
    t += 0.05
  }
...
rc.setAlpha(0.2)
noFill();
// l.rotate(radians(1),'z',createVector(0,0,0))
stroke(255,.050)
// ging.displayHL();
// r.display(ging)
// l2.display(ging)
// l.display(ging)
var pp = l2.randomPoints(10)
// console.log(pp)
for (let j = 0;j<pp.length;j++){
for (let i = 0;i<100;i++){
  var p2 = pp[j].copy()
  // p2.y -= Math.abs(randomGaussian(0,500))
  p2.z -= Math.abs(randomGaussian(0,1000))
  var p3 = ging.project([p2])[0]
  console.log(p3)
  var pc = color('#0e4c80')
  pc.setAlpha(0.1)
  stroke(pc)
