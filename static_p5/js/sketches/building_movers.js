...
function setup() {
  ...
  background('#ffe8b2');
  movers = [];
  params.noiseSeed = random(1000000)
  ...
  rec.translate(createVector(1000,0,20000))
  ...
  // shapes = arrayOnLine(rec,createVector(0,5000,0),createVector(0,0,2000),4)
  // shapes = makeArray([rec],500,20,'z')
  shapes = makeArray([rec],500,15,'y')
  shapes = makeArray(shapes,500,5,'x')
  shapes = makeArray(shapes,500,10,'z')

  for (let j = 0;j<shapes.length;j++){
    for (let i =0;i<1;i++){
    v = placePointOnPlane(shapes[j].faces[0],random(1),random(1))
  ...
  // shapes = makeArray(shapes,500,5,'z')
  console.log('shapes')
  console.log(shapes)
  var rott = 0
  // for (let i = 0;i<40;i++){
